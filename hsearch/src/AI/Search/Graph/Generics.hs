...
search :: (Num p, Ord i, Ord s) => (SNode s p -> a)                 -- ^ result function
...
search rf !md p !ct cg g s = join . maybeToList $ loop initFringe
    where initFringe = SG.singleton (p . SR.takeEnd) $ SR.singleton (SNode s 0 0)
          loop fr = do
            (cFr, routeTail SR.:> currentSNode@(SNode cs cd cc) ) <- SG.minNode fr
            let 
...
iterativeSearch :: (Num p, Ord i, Ord s) => (SNode s p -> a)         -- ^ result function
                           -> (SNode s p -> i)         -- ^ policy to be used
                           -> CheckTime                -- ^ when apply check goal
                           -> (s -> Bool)              -- ^ function used to check if the goal is reached
                           -> (s -> [(s, p)])          -- ^ generator of states
